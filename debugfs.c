...
static ssize_t mwl_debugfs_feature_read(struct file *file,
					char __user *ubuf,
					size_t count, loff_t *ppos)
{
	struct mwl_priv *priv = (struct mwl_priv *)file->private_data;
	...
	/* if activated, performance slightly downgraded.*/
...
static ssize_t mwl_debugfs_feature_write(struct file *file,
					    const char __user *ubuf,
					    size_t count, loff_t *ppos)
{
	struct mwl_priv *priv = (struct mwl_priv *)file->private_data;
...
MWLWIFI_DEBUGFS_FILE_OPS(feature);
...
void mwl_debugfs_init(struct ieee80211_hw *hw)
{
	...
	MWLWIFI_DEBUGFS_ADD_FILE(feature);
}
